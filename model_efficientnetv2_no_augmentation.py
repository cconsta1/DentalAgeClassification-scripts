...
# ============ Configurations ============ #
# Define directories with flexibility
ROOT_DIR = os.getenv("DATA_PATH", "./data")  # Users can set this via environment variables
DATASET_NAME = "DentAgePooledDatav2"
MODEL_DIR = os.getenv("MODEL_PATH", "./models/EfficientNetV2_no_augmentation")
...
print(f"Model: EfficientNetV2 with no augmentation. Timestamp: {timestamp}")
...
def get_basic_data(imgDir):
    ...
    dataset = CustomImageDataset(imgDir, transform)
...
train_loader, val_loader = get_basic_data(f"{ROOT_DIR}/{DATASET_NAME}")
